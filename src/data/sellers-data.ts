export const SELLERS = [
  {
    id: 1,
    avatarUrl: "/images/sellers/seller-1.png",
    rating: 4.6,
    name: "Orgânicos do Vale",
    description: "Produtor local de alimentos frescos e orgânicos.",
    slug: "organicos-do-vale",
    about:
      "A Orgânicos do Vale é uma empresa familiar que cultiva produtos orgânicos há mais de 20 anos, comprometida com a sustentabilidade e a saúde dos clientes.",
    address: {
      street: "Rua das Flores",
      number: "123",
      neighborhood: "Centro",
      city: "São Paulo",
      state: "SP",
      zipCode: "01000-000",
    },
    contact: {
      phone: "(11) 98765-4321",
      email: "contato@organicosdovale.com",
    },
    methodPayments: ["Cartão de Crédito", "Dinheiro", "Pix"],
    dateTimeWork: {
      days: ["Segunda", "Quarta", "Sábado"],
      hours: "08:00 - 14:00",
    },
    productsCategories: [
      {
        title: "Produtos em destaque",
        products: [
          {
            id: 1,
            title: "Cenoura Orgânica",
            type: "Legume",
            image: "/images/food/cenoura.png",
            price: 4.0,
            description: "Cenouras frescas, ricas em nutrientes.",
          },
          {
            id: 2,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
          {
            id: 3,
            title: "Salsa Orgânica",
            type: "Erva",
            image: "/images/food/salsa.png",
            price: 1.5,
            description: "Salsa fresca para temperar suas refeições.",
          },
        ],
      },
      {
        title: "Legumes",
        products: [
          {
            id: 4,
            title: "Abobrinha Orgânica",
            type: "Legume",
            image: "/images/food/abobrinha.png",
            price: 4.5,
            description: "Abobrinhas orgânicas de alta qualidade.",
          },
        ],
      },
      {
        title: "Verduras",
        products: [
          {
            id: 7,
            title: "Couve Orgânica",
            type: "Verdura",
            image: "/images/food/couve.png",
            price: 2.5,
            description: "Couve orgânica rica em vitaminas.",
          },
          {
            id: 8,
            title: "Espinafre Orgânico",
            type: "Verdura",
            image: "/images/food/beterraba.png",
            price: 3.2,
            description: "Espinafre orgânico perfeito para receitas saudáveis.",
          },
          {
            id: 9,
            title: "Alface Orgânica",
            type: "Verdura",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
        ],
      },
      {
        title: "Ervas",
        products: [
          {
            id: 10,
            title: "Manjericão Orgânico",
            type: "Erva",
            image: "/images/food/alface.png",
            price: 2.0,
            description: "Manjericão fresco para temperos.",
          },
          {
            id: 11,
            title: "Salsa Orgânica",
            type: "Erva",
            image: "/images/food/salsa.png",
            price: 1.5,
            description: "Salsa fresca para temperar suas refeições.",
          },
          {
            id: 12,
            title: "Cebolinha Orgânica",
            type: "Erva",
            image: "/images/food/cenoura.png",
            price: 1.8,
            description: "Cebolinha orgânica para adicionar sabor aos pratos.",
          },
        ],
      },
      {
        title: "Saladas",
        products: [
          {
            id: 13,
            title: "Rúcula Orgânica",
            type: "Salada",
            image: "/images/food/couve.png",
            price: 3.2,
            description: "Rúcula fresca, perfeita para saladas.",
          },
          {
            id: 14,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface crocante, ideal para saladas.",
          },
          {
            id: 15,
            title: "Acelga Orgânica",
            type: "Salada",
            image: "/images/food/salsa.png",
            price: 2.9,
            description: "Acelga fresca e crocante, ótima para pratos leves.",
          },
        ],
      },
    ],
  },
  {
    id: 2,
    avatarUrl: "/images/sellers/seller-2.png",
    rating: 4.8,
    name: "Sabor da Terra",
    description: "Cultivando sabores naturais direto do campo.",
    slug: "sabor-da-terra",
    about:
      "A Sabor da Terra oferece uma variedade de produtos frescos e orgânicos, colhidos com amor e dedicação por agricultores locais.",
    address: {
      street: "Avenida Verde",
      number: "456",
      neighborhood: "Jardim das Árvores",
      city: "Rio de Janeiro",
      state: "RJ",
      zipCode: "20000-000",
    },
    contact: {
      phone: "(21) 91234-5678",
      email: "contato@sabordaterra.com",
    },
    methodPayments: ["Dinheiro", "Cartão de Débito", "Transferência Bancária"],
    dateTimeWork: {
      days: ["Terça", "Quinta", "Domingo"],
      hours: "07:00 - 13:00",
    },
    productsCategories: [
      {
        title: "Produtos em destaque",
        products: [
          {
            id: 16,
            title: "Cenoura Orgânica",
            type: "Legume",
            image: "/images/food/cenoura.png",
            price: 4.0,
            description: "Cenouras frescas, ricas em nutrientes.",
          },
          {
            id: 17,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
          {
            id: 18,
            title: "Salsa Orgânica",
            type: "Erva",
            image: "/images/food/salsa.png",
            price: 1.5,
            description: "Salsa fresca para temperar suas refeições.",
          },
        ],
      },
      {
        title: "Legumes",
        products: [
          {
            id: 19,
            title: "Abobrinha Orgânica",
            type: "Legume",
            image: "/images/food/abobrinha.png",
            price: 4.5,
            description: "Abobrinhas orgânicas de alta qualidade.",
          },
          {
            id: 20,
            title: "Beterraba Orgânica",
            type: "Legume",
            image: "/images/food/beterraba.png",
            price: 3.0,
            description: "Beterrabas orgânicas cheias de sabor e nutrientes.",
          },
          {
            id: 21,
            title: "Pepino Orgânico",
            type: "Legume",
            image: "/images/food/pepino.png",
            price: 2.8,
            description: "Pepino crocante e fresco, ideal para saladas.",
          },
        ],
      },
      {
        title: "Verduras",
        products: [
          {
            id: 22,
            title: "Couve Orgânica",
            type: "Verdura",
            image: "/images/food/couve.png",
            price: 2.5,
            description: "Couve orgânica rica em vitaminas.",
          },
          {
            id: 23,
            title: "Espinafre Orgânico",
            type: "Verdura",
            image: "/images/food/beterraba.png",
            price: 3.2,
            description: "Espinafre orgânico perfeito para receitas saudáveis.",
          },
          {
            id: 24,
            title: "Alface Orgânica",
            type: "Verdura",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
        ],
      },
      {
        title: "Ervas",
        products: [
          {
            id: 25,
            title: "Manjericão Orgânico",
            type: "Erva",
            image: "/images/food/alface.png",
            price: 2.0,
            description: "Manjericão fresco para temperos.",
          },
          {
            id: 26,
            title: "Salsa Orgânica",
            type: "Erva",
            image: "/images/food/salsa.png",
            price: 1.5,
            description: "Salsa fresca para temperar suas refeições.",
          },
          {
            id: 27,
            title: "Cebolinha Orgânica",
            type: "Erva",
            image: "/images/food/cenoura.png",
            price: 1.8,
            description: "Cebolinha orgânica para adicionar sabor aos pratos.",
          },
        ],
      },
      {
        title: "Saladas",
        products: [
          {
            id: 28,
            title: "Rúcula Orgânica",
            type: "Salada",
            image: "/images/food/couve.png",
            price: 3.2,
            description: "Rúcula fresca, perfeita para saladas.",
          },
          {
            id: 29,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface crocante, ideal para saladas.",
          },
          {
            id: 30,
            title: "Acelga Orgânica",
            type: "Salada",
            image: "/images/food/salsa.png",
            price: 2.9,
            description: "Acelga fresca e crocante, ótima para pratos leves.",
          },
        ],
      },
    ],
  },
  {
    id: 3,
    avatarUrl: "/images/sellers/seller-3.png",
    rating: 4.9,
    name: "Raízes Naturais",
    description: "Produtos orgânicos e naturais direto da horta.",
    slug: "raizes-naturais",
    about:
      "Raízes Naturais é uma cooperativa de agricultores comprometidos com a produção sustentável e o cuidado com a natureza.",
    address: {
      street: "Estrada das Palmeiras",
      number: "789",
      neighborhood: "Zona Rural",
      city: "Belo Horizonte",
      state: "MG",
      zipCode: "30000-000",
    },
    contact: {
      phone: "(31) 99876-5432",
      email: "contato@raizesnaturais.com",
    },
    methodPayments: ["Pix", "Cartão de Crédito", "Dinheiro"],
    dateTimeWork: {
      days: ["Quarta", "Sexta", "Sábado"],
      hours: "09:00 - 15:00",
    },
    productsCategories: [
      {
        title: "Produtos em destaque",
        products: [
          {
            id: 31,
            title: "Cenoura Orgânica",
            type: "Legume",
            image: "/images/food/cenoura.png",
            price: 4.0,
            description: "Cenouras frescas, ricas em nutrientes.",
          },
          {
            id: 32,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
          {
            id: 33,
            title: "Salsa Orgânica",
            type: "Erva",
            image: "/images/food/salsa.png",
            price: 1.5,
            description: "Salsa fresca para temperar suas refeições.",
          },
          {
            id: 312,
            title: "Cenoura Orgânica",
            type: "Legume",
            image: "/images/food/cenoura.png",
            price: 4.0,
            description: "Cenouras frescas, ricas em nutrientes.",
          },
          {
            id: 322,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
          {
            id: 332,
            title: "Salsa Orgânica",
            type: "Erva",
            image: "/images/food/salsa.png",
            price: 1.5,
            description: "Salsa fresca para temperar suas refeições.",
          },
        ],
      },
      {
        title: "Legumes",
        products: [
          {
            id: 34,
            title: "Abobrinha Orgânica",
            type: "Legume",
            image: "/images/food/abobrinha.png",
            price: 4.5,
            description: "Abobrinhas orgânicas de alta qualidade.",
          },
          {
            id: 35,
            title: "Beterraba Orgânica",
            type: "Legume",
            image: "/images/food/beterraba.png",
            price: 3.0,
            description: "Beterrabas orgânicas cheias de sabor e nutrientes.",
          },
        ],
      },
      {
        title: "Verduras",
        products: [
          {
            id: 37,
            title: "Couve Orgânica",
            type: "Verdura",
            image: "/images/food/couve.png",
            price: 2.5,
            description: "Couve orgânica rica em vitaminas.",
          },
          {
            id: 38,
            title: "Espinafre Orgânico",
            type: "Verdura",
            image: "/images/food/beterraba.png",
            price: 3.2,
            description: "Espinafre orgânico perfeito para receitas saudáveis.",
          },
          {
            id: 39,
            title: "Alface Orgânica",
            type: "Verdura",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
          {
            id: 39,
            title: "Alface Orgânica",
            type: "Verdura",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
          {
            id: 39,
            title: "Alface Orgânica",
            type: "Verdura",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
        ],
      },
      {
        title: "Ervas",
        products: [
          {
            id: 40,
            title: "Manjericão Orgânico",
            type: "Erva",
            image: "/images/food/alface.png",
            price: 2.0,
            description: "Manjericão fresco para temperos.",
          },
        ],
      },
      {
        title: "Saladas",
        products: [
          {
            id: 43,
            title: "Rúcula Orgânica",
            type: "Salada",
            image: "/images/food/couve.png",
            price: 3.2,
            description: "Rúcula fresca, perfeita para saladas.",
          },
          {
            id: 44,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface crocante, ideal para saladas.",
          },
        ],
      },
    ],
  },
  {
    id: 4,
    avatarUrl: "/images/sellers/seller-4.png",
    rating: 4.7,
    name: "Vida Verde",
    description: "Sabor e saúde em cada colheita orgânica.",
    slug: "vida-verde",
    about:
      "Vida Verde é uma fazenda orgânica especializada em hortaliças e frutas de qualidade, com práticas agrícolas sustentáveis.",
    address: {
      street: "Alameda das Frutas",
      number: "321",
      neighborhood: "Bairro Rural",
      city: "Curitiba",
      state: "PR",
      zipCode: "80000-000",
    },
    contact: {
      phone: "(41) 98765-4321",
      email: "contato@vidaverde.com",
    },
    methodPayments: ["Cartão de Crédito", "Transferência Bancária", "Dinheiro"],
    dateTimeWork: {
      days: ["Segunda", "Quarta", "Sábado"],
      hours: "08:00 - 12:00",
    },
    productsCategories: [
      {
        title: "Produtos em destaque",
        products: [
          {
            id: 46,
            title: "Cenoura Orgânica",
            type: "Legume",
            image: "/images/food/cenoura.png",
            price: 4.0,
            description: "Cenouras frescas, ricas em nutrientes.",
          },
          {
            id: 47,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
          {
            id: 48,
            title: "Salsa Orgânica",
            type: "Erva",
            image: "/images/food/salsa.png",
            price: 1.5,
            description: "Salsa fresca para temperar suas refeições.",
          },
        ],
      },
      {
        title: "Legumes",
        products: [
          {
            id: 49,
            title: "Abobrinha Orgânica",
            type: "Legume",
            image: "/images/food/abobrinha.png",
            price: 4.5,
            description: "Abobrinhas orgânicas de alta qualidade.",
          },
          {
            id: 50,
            title: "Beterraba Orgânica",
            type: "Legume",
            image: "/images/food/beterraba.png",
            price: 3.0,
            description: "Beterrabas orgânicas cheias de sabor e nutrientes.",
          },
          {
            id: 51,
            title: "Pepino Orgânico",
            type: "Legume",
            image: "/images/food/pepino.png",
            price: 2.8,
            description: "Pepino crocante e fresco, ideal para saladas.",
          },
        ],
      },
      {
        title: "Verduras",
        products: [
          {
            id: 52,
            title: "Couve Orgânica",
            type: "Verdura",
            image: "/images/food/couve.png",
            price: 2.5,
            description: "Couve orgânica rica em vitaminas.",
          },
          {
            id: 53,
            title: "Espinafre Orgânico",
            type: "Verdura",
            image: "/images/food/beterraba.png",
            price: 3.2,
            description: "Espinafre orgânico perfeito para receitas saudáveis.",
          },
          {
            id: 54,
            title: "Alface Orgânica",
            type: "Verdura",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface orgânica crocante e fresca.",
          },
        ],
      },
      {
        title: "Ervas",
        products: [
          {
            id: 55,
            title: "Manjericão Orgânico",
            type: "Erva",
            image: "/images/food/alface.png",
            price: 2.0,
            description: "Manjericão fresco para temperos.",
          },
          {
            id: 56,
            title: "Salsa Orgânica",
            type: "Erva",
            image: "/images/food/salsa.png",
            price: 1.5,
            description: "Salsa fresca para temperar suas refeições.",
          },
          {
            id: 57,
            title: "Cebolinha Orgânica",
            type: "Erva",
            image: "/images/food/cenoura.png",
            price: 1.8,
            description: "Cebolinha orgânica para adicionar sabor aos pratos.",
          },
        ],
      },
      {
        title: "Saladas",
        products: [
          {
            id: 58,
            title: "Rúcula Orgânica",
            type: "Salada",
            image: "/images/food/couve.png",
            price: 3.2,
            description: "Rúcula fresca, perfeita para saladas.",
          },
          {
            id: 59,
            title: "Alface Orgânica",
            type: "Salada",
            image: "/images/food/alface.png",
            price: 3.5,
            description: "Alface crocante, ideal para saladas.",
          },
          {
            id: 60,
            title: "Acelga Orgânica",
            type: "Salada",
            image: "/images/food/salsa.png",
            price: 2.9,
            description: "Acelga fresca e crocante, ótima para pratos leves.",
          },
        ],
      },
    ],
  },
];

export type SellerProps = (typeof SELLERS)[0];
export type productsCategoriesProps = SellerProps["productsCategories"];
export type ProductProps = productsCategoriesProps[0]["products"][0];

export type Product = {
  id: string;
  title: string;
  type: string;
  image: string;
  price: number;
  description: string;
};

export type ProductsCategories = {
  title: string;
  products: Product[];
};

export type Seller = {
  id: number;
  avatarUrl: string;
  rating: number;
  name: string;
  description: string;
  slug: string;
  about: string;
  address: {
    street: string;
    number: string;
    neighborhood: string;
    city: string;
    state: string;
    zipCode: string;
  };
  contact: {
    phone: string;
    email: string;
  };
  methodPayments: string[];
  dateTimeWork: {
    days: string[];
    hours: string;
  };
  productsCategories: {
    title: string;
    products: {
      id: number;
      title: string;
      type: string;
      image: string;
      price: number;
      description: string;
    }[];
  }[];
};
